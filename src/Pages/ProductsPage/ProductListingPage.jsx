...
const ProductListingPage = () => {
  ...
  const [selectedBrand, setSelectedBrand] = useState("All"); // <-- ADDED
  ...
  const { categories, brands } = useMemo(() => {
    ...
    products.forEach((p) => {
      ...
      if (p.brand) allBrands.add(p.brand); // <-- ADDED
    });
    return {
      categories: [...allCategories],
      brands: [...allBrands], // <-- ADDED
    };
  }, [products]);
  ...
  const processedProducts = useMemo(() => {
    ...
    return filtered;
  }, [products, selectedCategory, selectedBrand, sortOption, searchTerm]); // <-- UPDATED DEPENDENCIES
  ...
  return (
    <div className="bg-gradient-to-br from-amber-50 to-orange-100 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {products.length === 0 ? (
          // Case 1: No products in the database at all
          ...
          // Case 2: Products exist, but filters match none
          <div className="flex flex-col justify-center items-center h-64 bg-white shadow-md rounded-lg p-6">
          ...
          // Case 3: Show the filtered and sorted products
